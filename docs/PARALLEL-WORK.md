...
**Step 1 never includes writing tests or taking a measurement** (D16, generalised in D23). Verification
is batched into the final phase, so any task contract's acceptance is "the code exists and does X", and
step 2 means the gate is still green on the suite that already exists — not that the task added to it. Two things that
