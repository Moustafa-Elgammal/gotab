...
| **V6.5** | ⚠️ **partial → Phase 7** | `internal/platform` behaviour | D46: enumeration ✓, raise ✓ for AX-visible / ✗ for `cg`-only → opened Phase 7. 2026-09-08 agent re-check (`spike/raisetest -v65`): **`both`-origin raise still works post-P7.1/P7.3** (no regression). Sentinels + minimized + hidden-app **inconclusive from an agent** — a standalone spike is not the `app.gotab` TCC identity (PLATFORM-LESSONS §5), so its AX calls time out; needs the bundle or a human. One thing to chase under V6.13: whether a `cg`-only window of a *hidden* app gets P7.1's fallback or `ErrNoWindow`. `ErrNotTrusted` / `ErrUnavailable`: not attempted. Contract + findings: `docs/tasks/V6.5.md` | a Mac, both grants |
...
| **V6.7** | ☐ ready | Ship the bundle (P4.5) | self-verified half done (D38). Machine half: `build/GoTab.app` launches from `/Applications` on a genuinely clean account (Gatekeeper needs a right-click → Open on a quarantined ad-hoc copy — or run `notarize.yml` once the `APPLE_*` secrets exist, D60, and it launches clean), and on a real macOS 12.0–12.2 host (`minos`/plist *agree*; a 12.0 binary *running* on 12.0 is untested — D17). Contract: `docs/tasks/V6.7.md` | a clean account + a macOS 12 host |
...
| **V6.10** | ⚠️ **partial (D51 → D58 → D59)** | VoiceOver heard, not just written (P5.2) | Structural half done via `gotab -switch -demo` + System Events: **per-tile a11y is right** — `AXButton` role, `"<title>, <app>"` label, exactly one `AXSelected` advancing on every cycle. D51's **container-label concern is fixed (D58):** `"Window switcher"` rides on the panel window's `AXTitle`, and the per-cycle announcement carries `"… , N of M"`. **Tile press now raises (D59)** — no longer a no-op; §6 flips from "confirm it does nothing" to "confirm a VO press switches and dismisses". **Still needs a human with VoiceOver:** container label heard on summon, selection + position spoken on **every** ⌥⇥ cycle, VO press-to-raise, and every settings control labelled. Contract + findings: `docs/tasks/V6.10.md` | a human |
...
- `2026-09-08` — **V6.5 remainder: `both`-origin raise re-confirmed post-Phase-7; the rest stays for
  a human.** Added `spike/raisetest -v65` (failure sentinels + minimized-window and hidden-app raise
  against throwaway TextEdit windows). Agent run: raising AX-visible windows still works after P7.1 /
  P7.3 (no regression), but the sentinels came back `ErrTimeout` not `ErrNoWindow` and the
  minimized/hidden cases did not resolve — a standalone spike run via `go run` is a fresh unsigned
  binary, not the `app.gotab` identity that holds the grants (PLATFORM-LESSONS §5), so its AX
  messaging times out. `-v65` is a human's turnkey (or a run from a granted stable binary), not an
  agent's. Flagged for V6.13: a leftover `cg`-only window of a *hidden* app returned `ErrNoWindow`
  from `Raise` rather than P7.1's app-activation fallback — possibly a stale straggler, possibly a
  gap where `pid_for_window` cannot resolve the owner; a human should confirm which. Gate green.
  **Next: unchanged — the human/hardware V6 checklist above.**
- `2026-09-08` — **V6.10 structural half done (D51); `gotab -switch -demo` added.** New `-demo` flag
  skips the hotkey and scripts a summon that holds + cycles, for an offscreen accessibility-tree
  walk (System Events — the Accessibility-Inspector substitute the contract allows). **Per-tile a11y
  verified:** `AXButton` role, `"<title>, <app>"` label, exactly one `AXSelected` advancing on every
  cycle (Chrome → GoLand → Mail sampled), press a no-op. **Concern for the VoiceOver run:** the
  `"Window switcher"` label sits on a view with `isAccessibilityElement = NO`, so the tiles are
  direct children of the panel window and its `AXTitle` is empty — the container may be voiceless. A
...
- `2026-09-08` — **VoiceOver: the panel's container label now reaches the tree (D58).** D51 found
  `"Window switcher"` was set on `GTTileView` but AppKit promoted the synthetic tile elements past
  that non-element view, so a screen reader landed on a title-less borderless panel. Fixed in
  `panel.m`: `gt_panel_create` sets the label as the panel window's `AXTitle` (spoken on focus-in)
  and pins `g_effect` as an accessibility pass-through; the per-cycle announcement (`a11y_spoken`)
  now carries `"<title>, <app>, N of M"` so a no-sight user always knows their place, independent of
  the container tree. No `panel.h` change. `check.sh` + `build.sh` (minos 12.0) green. V6.10 stays
  **partial** — a human VoiceOver pass is still owed (label heard on summon, selection + position on
  every cycle, settings controls). Tile press stays a no-op; press-to-raise is a separate follow-up
  (needs a new panel callback + an absolute-select event in `internal/app`).
- `2026-09-08` — **VoiceOver press-to-raise (D59).** A VoiceOver user navigating the panel tiles can
  now press one to switch to that window, instead of only the ⌥-release path. New `a11yaction.{h,go}`
  pair (frozen `panel.h` untouched, same trick as `timing.h`); `GTTileElement.accessibilityPerformPress`
  → `goPanelChooseTile(index)` → a new `Choose` event in `internal/app` that `Selection.Set`s the
