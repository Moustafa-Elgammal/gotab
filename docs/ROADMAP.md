...
| **V6.10** | ⚠️ **partial (D51 → D58)** | VoiceOver heard, not just written (P5.2) | Structural half done via `gotab -switch -demo` + System Events: **per-tile a11y is right** — `AXButton` role, `"<title>, <app>"` label, exactly one `AXSelected` advancing on every cycle, press a no-op (§6). D51's **container-label concern is now fixed (D58):** `"Window switcher"` rides on the panel window's `AXTitle` (AppKit was promoting the synthetic tiles past the non-element container view), and the per-cycle announcement carries `"… , N of M"`. **Still needs a human with VoiceOver:** that the container label is actually heard on summon, the selection + position on **every** ⌥⇥ cycle, and every settings control labelled. Contract + findings: `docs/tasks/V6.10.md` | a human |
