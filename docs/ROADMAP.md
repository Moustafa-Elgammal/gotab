...
| **V6.10** | ⚠️ **partial (D51)** | VoiceOver heard, not just written (P5.2) | Structural half done via `gotab -switch -demo` + System Events: **per-tile a11y is right** — `AXButton` role, `"<title>, <app>"` label, exactly one `AXSelected` advancing on every cycle, press a no-op (§6). **Concern:** `"Window switcher"` is on a view with `isAccessibilityElement = NO`, so the tiles are direct children of the panel window and the window's `AXTitle` is empty — a screen reader may not voice the container. **Needs a human with VoiceOver:** the container label, the settings control labels, and every announcement. Contract + findings: `docs/tasks/V6.10.md` | a human |
